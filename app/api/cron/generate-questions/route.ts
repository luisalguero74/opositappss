...
export async function GET(req: NextRequest) {
  try {
    ...
    const estadisticas = new Map(
      temasConPreguntas.map((t: { temaCodigo: string | null; _count: number }) => [t.temaCodigo?.toLowerCase(), t._count])
    )

    // Seleccionar temas con menos preguntas (máximo 5 por ejecución)
    const temasPendientes = TEMARIO_OFICIAL
      .filter((t: { id: string; numero: number; titulo: string }) => {
        const count = (estadisticas.get(t.id.toLowerCase()) || 0) as number
        return count < 100 // Generar hasta tener 100 preguntas por tema
      })
      .sort((a: { id: string }, b: { id: string }) => {
        const countA = (estadisticas.get(a.id.toLowerCase()) || 0) as number
        const countB = (estadisticas.get(b.id.toLowerCase()) || 0) as number
    ...
    await recordCronHeartbeat({
      ...
      details: {
        temasPendientes: temasPendientes.map((t: { id: string; numero: number; titulo: string }) => ({ id: t.id, numero: t.numero, titulo: t.titulo })),
        resultados
      }
    })
